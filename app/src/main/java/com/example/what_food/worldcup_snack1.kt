...
class worldcup_snack1 : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.worldcup_snack1)
        // round 체크
        var round = 8
        var now = 1
        var select = 0
        var next = 2
        // 음식 렌덤 이미지 8개
        var FoodImage = intArrayOf(R.drawable.jjolmyeon , R.drawable.kimbab, R.drawable.mando,
            R.drawable.odeng, R.drawable.ramen, R.drawable.sundae, R.drawable.tonkatsu, R.drawable.tteokbokki)
        ...
        // 음식  이름
        var FoodName = arrayOf("쫄면","김밥","만두","오뎅","라면","순대","돈까쓰", "떡볶이")
        ...
        right.setOnClickListener {
            ...
            {
                voteCount[select + 1]++
                var nextintent = Intent(this, worldcup_snack2::class.java)
                nextintent.putExtra("VoteCount", voteCount)
                nextintent.putExtra("ImageName", FoodName)
                nextintent.putExtra("WinFoodImage", FoodImage)
                startActivity(nextintent)
